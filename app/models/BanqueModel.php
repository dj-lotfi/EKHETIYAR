...
class BanqueModel extends Model
{
    ...
    {        
        ...
        if ($min != '' && $max != '') {
            $o = $this->_db->query('SELECT id_banque FROM `banque_prestation` bp INNER JOIN `prestations` p ON bp.id_prestation = p.id_prestation WHERE p.nom = ? AND ( (p.prix * p.iteration) BETWEEN ? AND ? ) AND p.prix IS NOT NULL', array($p, $min, $max),get_class($this)  );
        } elseif ($min != '' && $max == '') {
            $o = $this->_db->query('SELECT id_banque FROM `banque_prestation` bp INNER JOIN `prestations` p ON bp.id_prestation = p.id_prestation WHERE p.nom = ? AND (p.prix * p.iteration >= ?) AND p.prix IS NOT NULL', array($p, $min),get_class($this)  );
        } elseif ($min == '' && $max != '') {
